...
export default function Reports({ reportEntries, processorRoster, latestReportDate, historyVisible, historyEntries }: ReportsProps) {
    ...
    useEffect(() => {
        const parameters = new URLSearchParams(page.url.split('?')[1] ?? '');
        if (parameters.get('tab') === 'import') setPageTab('import');
        if (parameters.get('type') === 'midday' || parameters.get('type') === 'endOfDay') {
            setReportType(parameters.get('type') as ReportType);
        }
    }, [page.url]);
    ...
    const allRows = useMemo(() => {
        ...
        return processorRoster.map((processor) => {
            ...
            selectedReportEntries
                .filter((entry) => entry.processor_name === processor.name)
                .forEach((entry) => {
                        ...
                        <section className="overflow-hidden rounded-2xl border border-[#eadbc6] bg-[#fffdf8] shadow-[0_8px_30px_rgb(88,57,18,0.08)]">
                            ...
                            {hasReportData ? (
                                ...
                                <div className="flex min-h-72 flex-col items-center justify-center gap-4 px-6 py-12 text-center">
                                    <div className="grid size-16 place-items-center rounded-2xl border border-[#ead7b9] bg-[#fff5dc] text-[#a96300]">
                                        <FileSpreadsheet className="size-8" />
                                    </div>
                                    <div>
                                        <h3 className="text-lg font-extrabold text-[#342615]">No report data available</h3>
                                        <p className="mt-2 max-w-lg text-sm leading-6 text-[#806f59]">
                                            No saved Excel report records were found for {formatDate(reportDate)} and the selected report type. Import
                                            an Active or Closed workbook to generate this report.
                                        </p>
                                    </div>
                                    <Button
                                        type="button"
                                        onClick={() => setPageTab('import')}
                                        className="gap-2 bg-[#b96c00] font-bold text-white hover:bg-[#925400]"
                                    >
                                        <Upload className="size-4" /> Import Excel report
                                    </Button>
                                </div>
                            )}
                        </section>
